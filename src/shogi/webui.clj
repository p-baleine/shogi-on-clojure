  ...
  (html
   ...
   [:html
    [:head
     ...
     (include-js "/scripts/shogi.js")]
    [:body
     [:table
      [:tr
       [:td.first (make-captured-board :first)]
       [:td
        [:table.on-board
         (for [row (range 1 10)]
           [:tr
       ...
       [:td.second (make-captured-board :second)]
       ]]]]))
...
(defroutes main-routes
  ...
  (POST "/srv-make-move" {p :params}
        (srv-make-move!
         ...
         (Integer/parseInt (:to p)) (Boolean/valueOf(:promotion p))
         (session-get :board)))
  (GET "/srv-random-strategy" {p :params}
       (srv-random-strategy (keyword (:player p))))
  (GET "/srv-minimax-searcher" {p :params}
